...
def triangle_net(ub_triangle_node, ub_triangle_score, cand_triangle_node, cand_triangle_score, 
    ...
    with tf.variable_scope(name_or_scope=var_scp, reuse=reuse):
        ...
        ub_triangle_node_reshape = tf.reshape(ub_triangle_node, [-1, ub_tri_shape[1], int(ub_tri_shape[2] / 3), 3, ub_tri_shape[3]])
        ub_triangle_score_reshape = tf.reshape(ub_triangle_score, [-1, ub_tri_shape[1], int(ub_tri_shape[2] / 3), 1])
        # [B, SEQ_LENGTH, H]
        ...
        cand_triangle_node_reshape = tf.reshape(cand_triangle_node, [-1, 1, int(cand_tri_shape[1] / 3), 3, cand_tri_shape[2]])
        cand_triangle_score_reshape = tf.reshape(cand_triangle_score, [-1, 1, int(cand_tri_shape[1] / 3), 1])
        ...
        if use_negsampling:
            # luwei：这个应该对应paper里的Behavior Refiner Layer
...
def mhsa_inter_triangle_aggregation(inputs, num_units, num_heads, dropout_rate, name="", 
    ...
    #Luwei: [B*T*num_heads, TRIANGLE_NUM, TRIANGLE_NUM]
    ...
    # [B*T*num_heads, TRIANGLE_NUM, TRIANGLE_NUM]
    outputs = tf.where(tf.equal(key_masks, 0), paddings, align)
    outputs = tf.where(keys_empty_cond, paddings, outputs)
    outputs = tf.nn.softmax(outputs)   # [B*T*num_heads, TRIANGLE_NUM, TRIANGLE_NUM]
    # Attention Matmul
    ...
    # LUWEI：这里为啥要加这两层Dense？在DMIN里是没有的
...
def aux_loss_mhsa(inputs, num_units, num_heads, dropout_rate, name="", is_training=True, is_layer_norm=True):
    ...
    Q_K_V = tf.layers.dense(inputs, 3 * num_units)
    Q, K, V = tf.split(Q_K_V, 3, -1)
    Q_ = tf.concat(tf.split(Q, num_heads, axis=2), axis=0)  # [B*num_heads, T, num_units]
    K_ = tf.concat(tf.split(K, num_heads, axis=2), axis=0)  # [B*num_heads, T, num_units]
    V_ = tf.concat(tf.split(V, num_heads, axis=2), axis=0)  # [B*num_heads, T, num_units]
    
    outputs = tf.matmul(Q_, tf.transpose(K_, [0, 2, 1])) # [B*num_heads, T, T]
    align= outputs / (36 ** 0.5)
    # Diag mask 
    diag_val = tf.ones_like(align[0, :, :]) 
    tril = tf.linalg.LinearOperatorLowerTriangular(diag_val).to_dense() 
    
    key_masks = tf.tile(tf.expand_dims(tril, 0), [tf.shape(align)[0], 1, 1])  # [B*num_heads, T, T]
